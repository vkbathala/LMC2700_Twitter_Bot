...
function retweetLatest() {
	T.get('search/tweets', mediaArtsSearch, function (error, data) {
	  ...
	  if (!error) {
		...
		while (counter < length && translation.length + birdWords.length + 6 < 280) { // This loop goes until the tweet has 280 characters or all the words have
																											 // have been replaced with bird language
			birdWords += wordsOfBirds[Math.floor(Math.random()*wordsOfBirds.length)]; // using the Math functions, a number from 0 to wordsOfBirds.length, then is thranslated into a number 
																			   // that the array can handle
			counter++;
		}
		T.post('statuses/retweet/' + retweetId, {}, function (error, response) { // this creates a new post, but because of tweetLocation, 
		...
		var respond = {
			status: `@${userName}, I Like bids`,
			in_reply_to_status_id: retweetId
		};
		T.post('statuses/update', respond, function (error, response) { // this creates a new post, but because of tweetLocation, 
			if (response) {
		...
		//T.post('statuses/update', {status: translation + birdWords}, function (error, response) { // this creates a new post, but because of tweetLocation, 
			//if (response) {
